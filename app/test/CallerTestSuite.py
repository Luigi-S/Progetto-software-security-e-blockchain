...
def suite():
    ...
    suite.addTest(MethodCallTestCase('def test_invalid_address'))
    suite.addTest(MethodCallTestCase('def test_unmatching_abi'))
    suite.addTest(MethodCallTestCase('def test_valid'))
    #suite.addTest(MethodCallTestCase('def test_no_credit'))
    return suite
...
valid_port = int(os.environ.get("BASE_PORT"))
ganache_host = os.environ.get("GANACHE_HOST")
valid_link = f"ws://{ganache_host}:{valid_port}"
...
class MethodCallTestCase(unittest.TestCase):
    ...
    def test_unmatching_abi(self, stdout):
        ...
        Caller(chain_link=valid_link, contract_address=self.contract_address, abi=unmatching_abi) \
            .method_call(7, valid_address, matching_key)
        expected = "ERROR"
        self.assertTrue(expected in stdout.getvalue())
        
        Caller(chain_link=valid_link, contract_address=self.contract_address, abi=unmatching_abi) \
            .method_call(0, valid_address, matching_key)
    ...
    def test_valid(self):
        value = 3
        Caller(chain_link=valid_link, contract_address=self.contract_address, abi=v_abi).method_call(7, valid_address, matching_key)
